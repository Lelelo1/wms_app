...
class CustomerOrderQueries {
  static String many() =>
      "SELECT o.entity_id as id, concat(o.customer_firstname, ' ', o.customer_lastname) as name , o.increment_id as displayId, i.product_id as productId, i.qty_ordered as qtyOrdered, i.qty_picked as qtyPicked, o.created_at FROM sales_flat_order o JOIN sales_flat_order_item i ON i.order_id = o.entity_id JOIN catalog_product_entity p ON p.entity_id = i.product_id WHERE p.type_id = 'simple' AND status = 'pending' OR status = 'pendingpreorder'  OR status = 'processing' OR status = 'processingpreorder' ORDER BY o.created_at DESC LIMIT 30";
  static String single(int id) =>
      "SELECT o.entity_id as id, concat(o.customer_firstname, ' ', o.customer_lastname) as name , o.increment_id as displayId, i.product_id as productId, i.qty_ordered as qtyOrdered, i.qty_picked as qtyPicked, o.created_at FROM sales_flat_order o JOIN sales_flat_order_item i ON i.order_id = o.entity_id JOIN catalog_product_entity p ON p.entity_id = i.product_id WHERE o.entity_id = $id AND p.type_id = 'simple' AND (status = 'pending' OR status = 'pendingpreorder'  OR status = 'processing' OR status = 'processingpreorder') ORDER BY o.created_at DESC LIMIT 30";
...
class ProductQueries {
  ...
  static String fromSkuText(String skuText) =>
      "SELECT entity_id as id FROM `catalog_product_entity` WHERE `sku` LIKE '%$skuText%' AND `entity_id` NOT IN (SELECT `entity_id` FROM `catalog_product_entity_varchar` WHERE `attribute_id` = '283') LIMIT 20";
  //static String fromCustomerOrderId() =>
